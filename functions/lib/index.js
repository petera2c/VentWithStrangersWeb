...
const createVentLink = (vent, ventID) => {
    let link = "https://www.ventwithstrangers.com/problem/" +
        ventID +
        "/" +
        vent.title
            .replace(/[^a-zA-Z ]/g, "")
...
exports.newCommentListener = functions.database
    ...
    console.log(comment);
    return "";
    return createNotification(createVentLink(vent, ventID), "Your vent has a new comment!", "vent.userID");
});
exports.newPostListener = functions.database
    ...
    if (!vent)
        return "Error";
    return createNotification(createVentLink(vent, ventID), "Your new vent is live!", vent.userID);
});
